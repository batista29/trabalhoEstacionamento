                        ...
                        resp.forEach(r => {
                            if (r.id_vaga == v.id_vaga) {
                                if (r.hora_saida == "00:00:00") {
                                    ...
                                    var botaofina = document.querySelector(".btnCadVaga")
...
function calcularPreco() {
    const options7 = { method: 'GET' };
    fetch('http://localhost:3000/registros', options7)
        .then(response => response.json())
        .then(resp => {
            resp.forEach(r => {
                var valor = document.querySelector(".vaga").cloneNode(true);
                valor.classList.remove("modelo")

                let minuto = 60;
                var hora = new Date().toLocaleTimeString();
                let horaSaida = (parseInt(hora[0] * 10) + parseInt(hora[1]) * minuto) + parseInt(hora[3] * 10) + parseInt(hora[4])
                let entrada = (parseInt(r.hora_entrada[0] * 10) + parseInt(r.hora_entrada[1]) * minuto) + parseInt(r.hora_entrada[3] * 10) + parseInt(r.hora_entrada[4])
                let saida = (parseInt(r.hora_saida[0] * 10) + parseInt(r.hora_saida[1]) * minuto) + parseInt(r.hora_saida[3] * 10) + parseInt(r.hora_saida[4])
                if (saida == 0) {
                    let resultado = (horaSaida - entrada) * valorMinuto;
                    valor.querySelector(".valorPagar").innerHTML = resultado;
                } else {
                    let resultado = (saida - entrada) * valorMinuto;
                    valor.querySelector(".valorPagar").innerHTML = resultado;
                }
                var botaofina = document.querySelector(".btnCadVaga")
                valor.appendChild(botaofina);
                console.log(valor);
                document.querySelector(".cards").appendChild(valor);
            })
